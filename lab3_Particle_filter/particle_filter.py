...
class Simulator(object):
	...
	def moveParticles(self, particles, uk):
		...
		# TODO: Change this value for the correct one
		prior_particles = np.zeros_like(particles)
		for i in range(len(particles)):
			prior_particles[i] = particles[i] + uk + self.param.odometry_stdev * np.random.randn()
			if self.param.circular_hallway:
				prior_particles[i] = np.mod(prior_particles[i], self.param.domain)
			else:
				if prior_particles[i] > self.param.domain:
					prior_particles[i] = self.param.domain - np.mod(prior_particles[i], self.param.domain)
				elif prior_particles[i] < 0:
					prior_particles[i] *= -1
				else:
					pass
	...
	def resampleParticles(self, prior_particles, particle_weights):
		...
		# Method 1: Accumulative weights, locate each random number in one interval
		Sum = np.sum(particle_weights)
		particle_weights = particle_weights / Sum
		Accu_w = np.cumsum(particle_weights)
		randNs = np.random.random(N)
		resample_IDs = np.searchsorted(Accu_w,randNs)
		resampled_particles = prior_particles[resample_IDs]

		# Method 2: Resampling wheel
		# index = np.random.randint(0,N)
		# beta = 0
		# Max = np.max(particle_weights)
		# resampled_particles = []
		# for i in range(N):
		# 	beta += np.random.random() * 2*Max
